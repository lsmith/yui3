...
function EventFacade(type, target, payload) {
    ...
    if (payload) {
        ...
        if (isObject(payload, true) && !isArray(payload)) {
            ...
            // To protect against facade.details[0].someProp = 'x' causing
            // facade.data.someProp === 'x' due to this.data = data being pass
            // by ref.
            data = Y.Object(data);
        }
    }
    ...
    subscribe: function (target, args, details) {
        ...
        if (!abort) {
            ...
            if (abort) {
                ...
                // Register the subscription unless it's a pre-fired fireOnce
...
CustomEvent.FacadeEvent = new CustomEvent({
    ...
    **/
    fire: function (target, type) {
        ...
        //   been fired
        if (!hasSubs && (fired || (!fireOnce && !this.defaultFn))) {
            return;
        }
        ...
        if (fireOnce && !fired) {
            ...
            yuievt.fired[type] = (event.stopped === 2) ? ABORT : args;
        }
    },
